...
class DAVIS_train(Dataset):
    def __init__(self, time, size):
        ...
        self.file_names = sorted(os.listdir(self.data_path + "/JPEGImages/480p/crossing/"))
        self.time = time
    ...
    def _randseq_(self, file_names, idx):
        ...
        for time in range(self.time):
            image = self.trans(Image.open(self.data_path + "JPEGImages/480p/crossing/" \
                                          + file_names[idx]))
            mask = self.trans4(Image.open(self.data_path + "/Annotations_unsupervised/480p/crossing/" \
                                          + file_names[idx][:-3] + "png").convert('RGB'))
            images.append(image)
            masks.append(mask)
    ...
    def __getitem__(self, idx):
        images, masks = self._randseq_(self.file_names, idx)
...
def Loader(size, batch_size, time, num_workers, shuffle=False):
