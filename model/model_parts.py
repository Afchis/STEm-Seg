...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        B_size, Ch_size, H_size, W_size = x.size()
        c4 = self.layer0(x)
        c8 = self.layer1(c4)
        c16 = self.layer2(c8)
        c32 = self.layer3(c16)
...
class Decoder(nn.Module):
    def __init__(self, out_channel):
        super().__init__()
        self.squeeze32_0 = SqueezeBlock(in_channel=256, out_channel=128)
        self.squeeze32_1 = SqueezeBlock(in_channel=128, out_channel=64)
        self.squeeze32_2 = SqueezeBlock(in_channel=64, out_channel=32)

        self.squeeze16_0 = SqueezeBlock(in_channel=256, out_channel=128)
        self.squeeze16_1 = SqueezeBlock(in_channel=128, out_channel=64)
        self.conv16_2 = nn.Conv2d(32+64, 32, kernel_size=(1, 1))

        self.squeeze8_0 = SqueezeBlock(in_channel=256, out_channel=128)
        self.conv8_1 = nn.Conv2d(32+128, 32, kernel_size=(1, 1))
        ...
        self.conv4_1 = nn.Conv2d(32+128, 32, kernel_size=(1, 1))
        self.conv4_final = nn.Conv2d(32, out_channel, kernel_size=(1, 1))
...
if __name__ == '__main__':
    images = torch.randn([1, 3, 256, 256]) # t_min = 8
    enc = Encoder()
