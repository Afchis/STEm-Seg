...
class AccumData():       
    ...
    def tensorboard(self, writer, tb, epoch):
        ...
        train_metric = self.disp["train_metric"]/self.disp["iter"]
        if tb is not "None":
            writer.add_scalars('%s_Total_loss' % tb, {'train' : train_Tloss}, epoch)
            ...
            writer.add_scalars('%s_IoU_metric' % tb, {'train' : train_metric}, epoch)
    ...
    for epoch in range(args.epochs):
        ...
        if args.train:
            ...
            for i, data in enumerate(data_loader["train"]):
                ...
                accum_data.update("train_metric", train_metric)
